...
public function edit($id)
{
    ...
    $boms = BOMParts::where('product_id', $id)
            ->with('processTeam')
            ->get();

    $packingBoms = ProductComponents::get();
...
public function update(Request $request, $id)
{
    ...
    if ($request->hasFile('design_sheet')) {
        ...
        $designSheetImageUrl = $product->design_sheet;
    }
    ...
    if ($request->hasFile('data_sheet')) {
        ...
        $dataSheetImageUrl = $product->data_sheet;
    }
    ...
    $product->update([
        ...
        'design_sheet'  => $designSheetImageUrl,
        'data_sheet'    => $dataSheetImageUrl,
    ]);

    $oldBoms = BOMParts::where('product_id', $product->id)->pluck('id');
    ...
    foreach ($request->bom_part_name as $index => $productName) {
        ...
        $bom->bom_name = $productName;
        $bom->bom_category = ($request->bomType[$index] == 1)
            ? $request->bom_category[$index]
            : $categoryName->component_name;

        $bom->bom_unit = $request->bom_unit[$index] ?? null;
        $bom->bom_qty = $request->bom_quantity[$index] ?? 0;
        $bom->bom_price = $request->price[$index] ?? 0;
        $bom->bom_type = ($request->bomType[$index] == 1) ? 'production' : 'packing';
        $bom->production_bom_id = ($request->bomType[$index] == 2)
            ? $request->packing_bom[$index]
            : null;
        ...
        foreach ($processTeams as $row => $processTeam) {
            ...
            $bomProcessTeam->stage = "stage_" . ($row + 1);
            $bomProcessTeam->bom_id = $bom->id;
            $bomProcessTeam->product_id = $product->id;
            $bomProcessTeam->team_id = $processTeam;
            $bomProcessTeam->save();
        }
    }
